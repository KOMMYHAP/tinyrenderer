...
int main(int argc, char** argv)
{
	if (2 == argc)
	{
		model = make_unique<Model>(argv[1]);
	}
	else
	{
		model = make_unique<Model>("../obj/african_head/african_head.obj");
	}

	auto zbuffer = vector<float>(width * height, -std::numeric_limits<float>::max());
